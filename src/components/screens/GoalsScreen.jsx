...
const s = {
  page: { fontFamily: "'Georgia','Times New Roman',serif", maxWidth: 560, margin: '0 auto', padding: '2rem 1.25rem' },
  ...
  tag: (status) => ({
    ...
    border: `1px solid ${status === 'completed' ? 'var(--ts-done-border)' : 'var(--ts-step-border)'}`,
  }),
};
...
function GoalForm({ onSave, onCancel, initial = {} }) {
  ...
  const [step, setStep] = useState(0); // 0 = basic, 1 = clarify

  function set(key, val) { setForm(f => ({ ...f, [key]: val })); }

  return (
    <div style={{ ...s.card, background: 'var(--ts-step-bg)', border: '1px solid var(--ts-step-border)' }}>
      {step === 0 && (
        <>
          <span style={s.label}>What's your goal?</span>
          <input
            style={s.input}
            placeholder="e.g. Run 2km regularly"
            value={form.title}
          ...
          <textarea
            style={s.textarea}
            placeholder="Describe it a little... (optional)"
            value={form.description}
            onChange={e => set('description', e.target.value)}
          />
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={s.btn} onClick={() => form.title.trim() && setStep(1)}>
              Next →
            </button>
      ...
      {step === 1 && (
        <>
          <p style={{ ...s.muted, marginBottom: 16 }}>
            A few questions to help you think this through. Skip any that don't feel relevant.
          </p>
...
function GoalCard({ goal, onEdit, onDelete, onSelect, isSelected }) {
  ...
  function markGoalComplete() {
    dispatch({
      ...
      payload: { id: goal.id, data: { status: 'completed', completedAt: new Date().toISOString() } },
    });
  }
  ...
  function pauseGoal() {
    dispatch({
      type: actions.UPDATE_GOAL,
      payload: { id: goal.id, data: { status: goal.status === 'paused' ? 'active' : 'paused' } },
    });
  }
  ...
  return (
    <div style={{ ...s.card, border: isSelected ? '1.5px solid var(--ts-accent-text)' : '1px solid var(--ts-border)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            <span style={s.tag(goal.status)}>{goal.status}</span>
          </div>
          {goal.why && (
            <p style={{ ...s.muted, fontSize: 13, fontStyle: 'italic' }}>"{goal.why}"</p>
          )}
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <button style={s.btnGhost} onClick={() => setExpanded(e => !e)}>
          {expanded ? 'Hide steps' : `Steps (${totalSteps})`}
        </button>
        {!isSelected && goal.status === 'active' && (
          <button style={s.btnGhost} onClick={() => onSelect(goal.id)}>Set as focus</button>
        )}
        {isSelected && (
          <span style={{ ...s.tag('active'), alignSelf: 'center' }}>★ Focus</span>
        )}
        <button style={s.btnGhost} onClick={pauseGoal}>
          {goal.status === 'paused' ? 'Resume' : 'Pause'}
        </button>
        {goal.status !== 'completed' && (
          <button style={s.btnGhost} onClick={markGoalComplete}>Complete</button>
        )}
      ...
      {expanded && (
        <div style={{ marginTop: 14 }}>
          {goal.tinySteps.length === 0 && !addingStep && (
            <p style={{ ...s.muted, fontSize: 13, marginBottom: 10 }}>No tiny steps yet. What's the smallest thing you could do?</p>
          )}
          {goal.tinySteps.map(step => (
            <div key={step.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--ts-border)' }}>
              <button
                ...
                style={{
                  width: 20, height: 20, borderRadius: 4, border: '1.5px solid var(--ts-border)',
                  background: step.status === 'completed' ? 'var(--ts-btn)' : 'transparent',
              ...
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontSize: 14, color: 'var(--ts-heading)', textDecoration: step.status === 'completed' ? 'line-through' : 'none', opacity: step.status === 'completed' ? 0.6 : 1 }}>
                  {step.title}
                </p>
                {step.description && (
                  <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--ts-muted)' }}>{step.description}</p>
                )}
              </div>
          ...
          {addingStep ? (
            ...
            <button style={{ ...s.btnGhost, marginTop: 10, width: '100%' }} onClick={() => setAddingStep(true)}>
              + Add tiny step
            </button>
          )}
        </div>
...
export default function GoalsScreen() {
  ...
  function selectGoal(id) {
    dispatch({ type: actions.SET_CURRENT_GOAL, payload: id });
  }
  ...
  return (
    <div style={s.page}>
      ...
      {/* Edit goal form */}
      {editingGoal && (
        <GoalForm initial={editingGoal} onSave={saveGoal} onCancel={() => setEditingGoal(null)} />
      )}
      ...
      {filtered.length === 0 ? (
        <div style={{ ...s.card, textAlign: 'center', padding: '2rem' }}>
          <p style={s.muted}>
            {goals.length === 0
              ? 'No goals yet. What's something you want to work towards?'
              : `No ${filter} goals.`}
          </p>
        </div>
      ) : (
